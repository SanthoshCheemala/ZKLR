...
def main():
    ...
    print(f"Train: {train_pos}/{args.train} positive ({100*train_pos/args.train:.1f}%)")
    ...
    for size in test_sizes:
        ...
        test_path = DATA_DIR / f"test_{size}.csv"
        save_csv(test_data, test_path)
        
        test_acc = evaluate(test_data, w, b)
        print(f"  Test {size}: {test_acc:.2%} accuracy")
