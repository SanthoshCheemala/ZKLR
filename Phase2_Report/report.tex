...
\textbf{Phase 2} introduces three major optimizations. First, the sigmoid lookup table range was tightened from $\pm16$ to $\pm10$, reducing circuit constraints by 32\% (from 159,804 to 114,219), compile time by 34\%, and setup time by 47\%. Second, a parallelized batch proving architecture was introduced, amortizing setup costs across datasets of any size and achieving a 2.2$\times$ wall-clock speedup over sequential proving for 100-sample datasets. Third, the circuit was generalized to support $N$-feature logistic regression models natively through dynamic slice-based constraint generation. A smart contract integration design was also documented, demonstrating how the 584-byte BN254 PLONK proofs can be verified on-chain via an auto-exported Solidity verifier contract.

Experimental evaluation on an Apple Silicon M-series laptop demonstrates a wall-clock proving speed of 0.917 seconds per sample (batch=40, 77\% classification accuracy), establishing ZKLR as a viable framework for privacy-preserving ML inference in resource-constrained environments.
...
\input{ch02_literature.tex}
\input{ch03_problem.tex}
\input{ch04_architecture.tex}
\input{ch05_implementation.tex}
\input{ch06_experiments.tex}
\input{ch07_results.tex}
\input{ch08_discussion.tex}
\input{ch09_applications.tex}
\input{ch10_future.tex}
\input{ch11_conclusion.tex}
